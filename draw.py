...
def scanline_convert(polygons, i, screen, zbuffer ):
    ...
    for x in range(len(polygons)):
        ...
        p2 = polygons[x+2]
        ary = [p0, p1, p2]
        sorted_y = [y for _,y in zip([z[2] for z in ary], ary)]
        bot = sorted_y[0]
        mid = sorted_y[1]
        too = sorted_y[2]
        delta0 = (top[0] - bot[0] )/(top[1] - bot[1])
        delata1 = (mid[0] - bot[0])/(mid[1] - bot[1])
        if bot[1] == mid[1]:
            delta1 = (top[0] - mid[0])/(top[1] - mid[1])
        else:
        
            y = bot[1] 
            while y < top[1]:
                draw_line(y*delta1,y,z,y*delta5,y,z, screen, zbuffer, color)
                y+=delta0
        x+=3
